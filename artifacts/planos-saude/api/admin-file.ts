...
async function github(path: string, init?: any) {
  ...
  if (!response.ok) {
    throw new Error(data && data.message ? data.message : `Erro GitHub: ${response.status}`);
  }
  return data;
}
...
export default async function handler(req: any, res: any) {
  try {
    if (!isAuthorized(req)) {
      return send(res, 401, { ok: false, message: 'Senha invalida.' });
    }

    const path = typeof req.query.path === 'string' ? req.query.path : DEFAULT_PATH;
    if (!isAllowedPath(path)) {
      return send(res, 400, { ok: false, message: 'Arquivo nao permitido para edicao.' });
    }
    ...
    if (req.method === 'POST') {
      ...
      if (typeof content !== 'string') {
        return send(res, 400, { ok: false, message: 'Conteudo invalido.' });
      }
