...
function isImagePath(path: string) { const lower = path.toLowerCase(); return lower.includes('logodataurl') || lower.includes('imagedataurl') || lower.includes('imageurl') || lower.includes('photourl') || lower.endsWith('.image') || lower.endsWith('.photo'); }
function blankClone(value: JsonValue): JsonValue { if (Array.isArray(value)) return []; if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([key, child]) => [key, key.toLowerCase().includes('image') || key.toLowerCase().includes('logo') ? '' : blankClone(child)])); if (typeof value === 'number') return 0; if (typeof value === 'boolean') return false; return ''; }
...
function ImageInput({ value, path, onChange }: { value: string; path: string; onChange: (path: string, value: JsonValue) => void }) {
  function upload(event: React.ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; if (file.size > 700_000) { alert('Use uma imagem menor que 700 KB para o site continuar leve.'); return; } const reader = new FileReader(); reader.onload = () => onChange(path, String(reader.result || '')); reader.readAsDataURL(file); }
