...
const sections: Section[] = [
  ...
  { id: 'gallery', label: 'Fotos', path: 'gallery' },
...
const labels: Record<string, string> = {
  ...
  metrics: 'Métricas', value: 'Valor', suffix: 'Sufixo', label: 'Rótulo', operators: 'Operadoras', items: 'Itens', title: 'Título', description: 'Descrição', imageDataUrl: 'Imagem', imageUrl: 'Imagem', photoUrl: 'Foto',
...
function isImagePath(path: string) {
  ...
  return lower.includes('logodataurl') || lower.includes('imagedataurl') || lower.includes('imageurl') || lower.includes('photourl') || lower.endsWith('.image') || lower.endsWith('.photo');
}

function blankClone(value: JsonValue): JsonValue {
  if (Array.isArray(value)) return [];
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, child]) => [key, key.toLowerCase().includes('image') || key.toLowerCase().includes('logo') ? '' : blankClone(child)]));
  }
...
function ImageInput({ value, path, onChange }: { value: string; path: string; onChange: (path: string, value: JsonValue) => void }) {
  function upload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 700_000) {
      alert('Use uma imagem menor que 700 KB para o site continuar leve.');
      return;
    }
...
function Preview({ content, section }: { content: any; section: string }) {
  ...
  if (section === 'gallery') return <div className="space-y-4 text-center"><p className="uppercase text-xs text-teal-700 font-bold">{content.gallery.kicker}</p><h3 className="text-3xl font-black">{content.gallery.title}</h3><p>{content.gallery.description}</p><div className="grid grid-cols-2 gap-3 text-left">{content.gallery.items.map((item: any, index: number) => <div key={index} className="overflow-hidden rounded-xl border bg-white"><div className="aspect-[4/3] bg-slate-100 flex items-center justify-center">{item.imageDataUrl ? <img src={item.imageDataUrl} className="h-full w-full object-cover" /> : <span className="text-xs text-slate-400">Sem imagem</span>}</div><div className="p-3"><b>{item.title}</b><p className="text-xs text-slate-500">{item.description}</p></div></div>)}</div></div>;
